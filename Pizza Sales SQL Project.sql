...
SELECT 
    ...
    ROUND(SUM(order_details.quantity * pizzas.price) / (SELECT 
        ...
        FROM
            order_details
                INNER JOIN
            pizzas ON order_details.pizza_id = pizzas.pizza_id)*100,2) AS percentage
FROM
    pizza_types
        INNER JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
FROM
    ...
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
        INNER JOIN
    order_details ON order_details.pizza_id = pizzas.pizza_id
GROUP BY pizza_types.name, pizza_types.category, pizza_types.pizza_type_id
ORDER BY revenue DESC) AS T) AS T1 
WHERE ranks<=3;